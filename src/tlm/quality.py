...
class BarQualityReport:
    ...
    end_timestamp: str | None
    duplicate_timestamps: int
    intraday_gap_rows: int
    invalid_ohlc_rows: int
    non_positive_tick_count_rows: int
    negative_spread_rows: int
    price_jump_rows: int
...
def build_bar_quality_report(
    ...
    missing_files = [str(path) for path in bar_files if not path.exists()]
    existing_paths = [path for path in bar_files if path.exists()]
    expected_files = len(bar_files)
    metadata = {
        "artifact": "bar_quality_report",
        "symbol": symbol,
        "timeframe": timeframe,
        "expected_files": [str(path) for path in bar_files],
        "expected_minutes": expected_minutes,
        "max_normal_price_jump": max_normal_price_jump,
    }
    data_version_hash = compute_data_version_hash(existing_paths, metadata)
    files = [str(path) for path in existing_paths]
    zero_row_files = [str(path) for path in existing_paths if _parquet_row_count(path) == 0]
    coverage_ratio = len(existing_paths) / expected_files if expected_files else 1.0
    if not files:
        quality_flags = _bar_quality_flags(
            missing_files=missing_files,
            zero_row_files=[],
            duplicate_timestamps=0,
            intraday_gap_rows=0,
            invalid_ohlc_rows=0,
            non_positive_tick_count_rows=0,
            negative_spread_rows=0,
            price_jump_rows=0,
        )
        ...
        row = con.execute(
            ...
            ordered_bars AS (
                SELECT
                    ...
                    timestamp,
                    close,
                    lag(timestamp) OVER (PARTITION BY symbol ORDER BY timestamp) AS previous_timestamp,
                    ...
                    SELECT coalesce(sum(
                        CASE
                            WHEN previous_timestamp IS NOT NULL
                                 AND date_trunc('day', previous_timestamp) = date_trunc('day', timestamp)
                                 AND date_diff('minute', previous_timestamp, timestamp) > ?
                            THEN 1 ELSE 0
                        END
                    ), 0)::INTEGER
                    FROM ordered_bars
                ) AS intraday_gap_rows,
                coalesce(sum(
                    CASE
                        ...
                        THEN 1 ELSE 0
                    END
                ), 0)::INTEGER AS invalid_ohlc_rows,
                coalesce(sum(CASE WHEN tick_count <= 0 THEN 1 ELSE 0 END), 0)::INTEGER AS non_positive_tick_count_rows,
                coalesce(sum(CASE WHEN avg_spread IS NOT NULL AND avg_spread < 0 THEN 1 ELSE 0 END), 0)::INTEGER AS negative_spread_rows,
                (
                    SELECT coalesce(sum(
                        CASE
                            WHEN previous_close IS NOT NULL
                                 AND abs(close - previous_close) > ?
                            THEN 1 ELSE 0
                        END
                    ), 0)::INTEGER
                    FROM ordered_bars
            ...
            FROM bars
            """,
            [files, expected_minutes, max_normal_price_jump],
        ).fetchone()
    finally:
        con.close()
    ...
    quality_flags = _bar_quality_flags(
        ...
        zero_row_files=zero_row_files,
        duplicate_timestamps=int(row[3]),
        intraday_gap_rows=int(row[4]),
        invalid_ohlc_rows=int(row[5]),
        non_positive_tick_count_rows=int(row[6]),
        negative_spread_rows=int(row[7]),
        price_jump_rows=int(row[8]),
    )
    return BarQualityReport(
        symbol=symbol,
...
def _bar_quality_flags(
    ...
    if intraday_gap_rows:
        flags.append("intraday_gaps")
    if invalid_ohlc_rows:
        flags.append("invalid_ohlc")
    if non_positive_tick_count_rows:
        flags.append("non_positive_tick_count")
    if negative_spread_rows:
        flags.append("negative_spread")
    if price_jump_rows:
        flags.append("price_jumps")
